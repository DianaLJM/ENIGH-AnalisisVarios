...
*/
gl dataenigh="D:\diana_jimenez\Documents\DATOS\ENIGH\Pobreza"
...
log using "$log/PobrezaProgsSoc_24.txt", text replace
...
use "$dataenigh/poblacion24.dta", clear
...
use "$dataenigh/trabajos24.dta", clear
...
label define discap 0 "Sin discapacidad" 1 "Con discapacidad", add
...
use "$dataenigh/ingresos24.dta", clear
...
use "$dataenigh/viviendas24.dta", clear
sort folioviv
save "$bases/viviendas24.dta", replace
use "$dataenigh/concentradohogar24.dta", clear
sort folioviv 
merge folioviv using "$bases/viviendas24.dta"
tab _merge
drop _merge
...
use "$dataenigh/hogares24.dta", clear
...
gen double ing_mon=ing_mens if (clave>="P001" & clave<="P009") | (clave>="P011" & clave<="P016") ///
               | (clave>="P018" & clave<="P041") | (clave>="P067" & clave<="P081") //| (clave>="P018" & clave<="P048") | (clave>="P067" & clave<="P081") 
...
use "$dataenigh/gastoshogar24.dta", clear
gen base=1
append using "$dataenigh/gastospersona24.dta"
recode base (.=2)
...
sort folioviv foliohog
save "$bases/tamhogesc24.dta", replace
...
merge folioviv foliohog using "$bases/tamhogesc24.dta"
tab _merge
drop _merge
...
label define plp_e 0 "Población con ingreso igual o mayor a la LPEI" ///
		  1 "Población con ingreso inferior a la LPEI", add
...
label define plp 0 "Población con ingreso igual o mayor a la LPI" ///
		 1 "Población con ingreso inferior a la LPI", add
label value plp plp
...
label define carencias 0 "Población sin carencias sociales" /// 
	        1 "Población con al menos una carencia social", add
...
label define carencias3 0 "Población con menos de tres carencias sociales" ///
			1 "Población con al menos tres carencias sociales", add

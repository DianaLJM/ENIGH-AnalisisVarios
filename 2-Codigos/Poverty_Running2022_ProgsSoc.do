...
*/
/*
gl data="/Volumes/T7 Shield/DATOS_brutos/ENIGH"
gl bases="/Volumes/TRABAJO23/2023/Datos/7-Julio/SolicitudesUPEHP/Pobreza/Procesado"
gl log="/Volumes/TRABAJO23/2023/Datos/7-Julio/SolicitudesUPEHP/Pobreza/Procesado/Log"
*/
gl data="D:\diana_jimenez\Documents\DATOS\ENIGH\Pobreza"
gl bases="F:\2025\Datos\8-Agosto\ENIGH\Pobreza\2-Results\Simulaciones"
gl log="F:\2025\Datos\8-Agosto\ENIGH\Pobreza\2-Results\Simulaciones"

log using "$log/Pobreza_22.txt", text replace // 2 B changed when program is updated FINALLY
...
use "$data/poblacion22.dta", clear
...
label define inas_esc  0 "Sí asiste" 1 "No asiste", add
...
label define niv_ed  0 "Con primaria incompleta o menos"  ///
					 ...
					 3 "Media superior completa o mayor nivel educativo", add
...
label define caren 0 "No presenta carencia" 1 "Presenta carencia", add
...
label define hli 0 "No habla lengua indígena"  1 "Habla lengua indígena", add
label value hli hli
...
label define ocupa 0 "Sin ocupación secundaria" 1 "Con ocupación secundaria", add
...
label define tipo_trab 1 "Depende de un patrón, jefe o superior" ///
                       ...
                       3 "No depende de un jefe y no recibe o no tiene asignado un sueldo", add
...
label define cuenta 0 "No cuenta" 1 "Sí cuenta", add
...
label define sa_dir 0 "Sin acceso" 1 "Con acceso", add
...
de acuerdo con el cuestionario de la ENIGH 2020. // 2 B verified in ENIGH 2022
*****************************************************************************
*/
...
label define discap  0 "Sin presencia de discapacidad" ///
                     1 "Con presencia de discapacidad", add
...
label define ocupa   0 "Sin ocupación secundaria"  ///
                     1 "Con ocupación secundaria", add
label value ocupa2 ocupa
...
recode ing_pens (.=0) //ing_pam (.=0)

collapse (sum) ing_pens, by(folioviv foliohog numren) //ing_pam, by(folioviv foliohog numren)
...
label define jub 0 "Población no pensionada o jubilada"  ///
                 1 "Población pensionada o jubilada", add
label value jub jub
...
label define ss_dir 0 "Sin acceso" 1 "Con acceso", add
...
label define pam 0 "No recibe" 1 "Recibe", add
label value pam pam
*/
*Indicador de carencia por acceso a la seguridad social
************************************************************************
...
use "$data/viviendas22.dta", clear
sort folioviv
save "$bases/viviendas22.dta", replace
use "$data/concentradohogar22.dta", clear
sort folioviv 
merge folioviv using "$bases/viviendas22.dta"
tab _merge
drop _merge
...
label define caren 0 "Sin carencia" 1 "Con carencia", add
...
label define id_men 0 "Sin población de 0 a 17 años" ///
                    1 "Con población de 0 a 17 años", add
...
label define si_no 0 "No" 1 "Sí", add
...
label define ins_ali 0 "Seguridad alimentaria" ///
							  ...
							  3 "Inseguridad alimentaria severa", add
...
lab def lca 0 "No limitado" 1 "Limitado", add
lab val lca lca
...
sort folioviv foliohog numren
save "$bases/aguinaldo22.dta", replace
...
merge folioviv foliohog numren using "$bases/aguinaldo22.dta"
tab _merge
drop _merge
...
gen double ing_mon=ing_mens if (clave>="P001" & clave<="P009") | (clave>="P011" & clave<="P016")  ///
                             | (clave>="P018" & clave<="P041") | (clave>="P067" & clave<="P081")
...
*las claves de ingreso correspondientes
gen double ing_tra=ing_mens if (clave>="P032" & clave<="P041") 
//(clave>="P032" & clave<="P048") 
												    	  //| (clave>="P101" & clave<="P108") 
...
use "$data/gastoshogar22.dta", clear
gen base=1
append using "$data/gastospersona22.dta"
recode base (.=2)
...
*Rubro 1.1 semanal, Alimentos			(es mensual)
...
*Rubro 1.2 semanal, Bebidas alcohólicas y tabaco			(es mensual)
...
sort folioviv foliohog
save "$bases/tamhogesc22.dta", replace
...
merge folioviv foliohog using "$bases/tamhogesc22.dta"
tab _merge
drop _merge
...
label define no_pobv 0 "Pobre o vulnerable"  ///
					 1 "No pobre y no vulnerable", add
...
label define carencias 0 "Población sin carencias"  ///
					   1 "Población con al menos una carencia", add
...
label define carencias3 0 "Población con menos de tres carencias"  ///
						1 "Población con al menos tres o más carencias", add
...
label define cuadrantes 1 "Pobres" ///
						...
						4 "No pobres y no vulnerables", add
